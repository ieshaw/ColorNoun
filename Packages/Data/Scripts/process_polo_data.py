...
raw_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..')) + '/CSVs/Raw/Polo/'

out_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..')) + '/CSVs/Processed/Polo/'
...
for file in os.listdir(raw_dir):
    ...
    coin_df.index = coin_df['date']

    x_df = pd.DataFrame(index= coin_df['date'])

    x_df['spread_{}'.format(coin)] = coin_df['high'] - coin_df['low']

    x_df[['volume_{}'.format(coin) ,'quoteVolume_{}'.format(coin)]] = coin_df[['volume' ,'quoteVolume']]
